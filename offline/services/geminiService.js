...
export class AICoachService {
    async *getSuggestionStream(mode, step, currentData, prompt) {
        ...
        const isFilePreview = typeof window !== "undefined" && window.location?.protocol === "file:";
        if (isFilePreview) {
        ...
        try {
            const resp = await fetch("/api/coach", {
                ...
                body: JSON.stringify({
                    mode,
                    step,
                    currentData: currentData?.[step] ?? currentData,
                    prompt,
                }),
            });
            if (!resp.ok) {
